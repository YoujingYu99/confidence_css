...
audio_path = os.path.join(fileDir, 'Spotify-Podcasts','podcasts-audio-only-2TB','podcasts-audio','0')
...
pydub.utils.get_prober_name = lambda: os.path.join(home_dir, 'confidence_css', 'ffprobe.exe')
        ...
        if audio_name in ogg_file_list:
            ...
            audio_file_path = os.path.join(audio_path,
                                           ...
                                           str(ogg_file_up_list[ogg_index]),
                                               audio_name)
            audio_excerpt_name = os.path.join(excerpt_output_path,
                                              str(ogg_file_sub_list[
                                                      ogg_index]),
                                              str(ogg_file_up_list[ogg_file_list.index(audio_name)]),
                                              str(audio_name + str(start)))
...
def extract_all_audio_path(audio_folder_series_path):
    ...
    for top_folder_path in audio_folder_series_path:
        middle_folder_path_list = [x for x in
                                   next(os.walk(top_folder_path))[1]]
        for middle_folder_path in middle_folder_path_list:
            bottom_folder_path_list = [x for x in next(
                os.walk(os.path.join(top_folder_path, middle_folder_path)))[1]]
            for bottom_folder_path in bottom_folder_path_list:
                for filename in os.listdir(
                        os.path.join(top_folder_path, middle_folder_path,
                                     bottom_folder_path)):
                    if filename.endswith('ogg'):
                        audio_path_list.append(
                            os.path.join(top_folder_path, middle_folder_path,
                                         bottom_folder_path, filename))
