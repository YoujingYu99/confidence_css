...
class CustomBERTSimpleModel(nn.Module):
    ...
    def forward(self, input_id, mask):
        ...
        dropout_norm = self.layernorm1(dropout)
        # print("pooled size", pooled_output.size())
        linear1 = self.linear1(dropout_norm)
...
class CustomHUBERTSimpleModel(nn.Module):
    ...
    def forward(self, input_values):
        output_tuple = self.hubert(input_values=input_values, return_dict=False)
        (pooled_output,) = output_tuple
        pooled_hubert = torch.mean(pooled_output, dim=1)

        dropout = self.dropout(pooled_hubert)
        dropout_norm = self.layernorm1(dropout)
        # print("pooled size", pooled_output.size())
        linear1 = self.linear1(dropout_norm)
        # print("linear 1", linear1.size())
...
class CustomMultiModelSimplePooledThreeLayers(nn.Module):
    def __init__(self, dropout=0.5):
        super(CustomMultiModelSimplePooledThreeLayers, self).__init__()
        self.bert = BertModel.from_pretrained("bert-base-cased")
        self.hubert = HubertModel.from_pretrained("facebook/hubert-base-ls960")
        self.relu = nn.ReLU()
        self.layernorm1 = nn.LayerNorm([4, 768 * 2])
        ...
        self.layernorm3 = nn.LayerNorm([4, 8])
    ...
    def forward(self, input_values, input_id, mask):
        ...
        dropout3 = self.dropout(relu2)
        linear2_norm = self.layernorm3(dropout3)

        linear3 = self.linear3(linear2_norm)
        # print("linear 2 size", linear2.size())
        tanh = self.tanh(linear3)
        # Scale to match input
        prediction = tanh * 2.5
        # print("prediction size", prediction.size())
