...
# Read in only csv file; audio without features
...
audio_df = dd.read_csv(
    total_audio_file, converters={"audio_array": pd.eval}, encoding="utf-8"
)
...
print(audio_df.head())
print(type(audio_df))
# df_train, df_val, df_test = np.split(audio_df.sample(frac=1, random_state=42),[int(0.8 * len(audio_df)), int(0.9 * len(audio_df))],)
...
# Train model
train_audio_audio(audio_model, df_train, df_val, LR, EPOCHS)
# # home_dir is the location of script
# home_dir = os.path.join("/home", "yyu")
#
