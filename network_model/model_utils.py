...
def evaluate_text(model, test_data, tokenizer):
...
class AudioDatasetVector(torch.utils.data.Dataset):
    """
...
class AudioDatasetAudio(torch.utils.data.Dataset):
    """
    Prepare the audio dataset according to its attributes.
    """
    ...
    def pad_audio(self):
        ...
        for audio_array in self.audios_list:
            ...
            # padded_audio_array = audio_array + [pad_token] * (self.max_length - len(audio_array))
...
def train_audio_vector(
    model, feature_extractor, train_data, val_data, learning_rate, epochs
):
    """Train the model based on extracted audio features."""
    train, val = train_data.reset_index(drop=True), val_data.reset_index(drop=True)
    train, val = (
        AudioDatasetVector(train, feature_extractor),
        AudioDatasetVector(val, feature_extractor),
    )
...
def train_audio_audio(model, train_data, val_data, learning_rate, epochs):
    """Train the model based on extracted audio only."""
    train, val = train_data.reset_index(drop=True), val_data.reset_index(drop=True)
    train, val = (
        AudioDatasetAudio(train),
        AudioDatasetAudio(val),
    )
...
def evaluate_audio_vector(model, test_data):
...
def evaluate_audio_audio(model, test_data):
    """Evaluate accuracy for audio data."""
    test = test_data.reset_index(drop=True)
    ...
    with torch.no_grad():
        ...
        for test_input, test_label in test_dataloader:
            test_label = test_label.to(device)
            # mask = test_input["attention_mask"].to(device)
            input_id = test_input["input_values"].to(device)
